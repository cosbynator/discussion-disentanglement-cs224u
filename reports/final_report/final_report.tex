...
Named Entities & 0.44 & 0.46 & 0.64 & 0.12 & 0.12 & 0.16 \\
   ...
   % Increases as we go down in depth
               Message Length & 0.15 & 0.17 & 0.37 & 0.38 & 0.35 & 0.33 \\
   \hline
  \end{tabular}
...
\begin{table}[h]\footnotesize
 \begin{tabular}{| l | l | l | l | l | l | c |} 
   \hline
   \textbf{Test} & \multirow{2}{*}{\shortstack{Tree\\F1}} & \multirow{2}{*}{\shortstack{Avg.\\F1}} & \multicolumn{4}{c|}{$B^3$ depth} \\
                      & & & 1 & 2 & 3 & 4\\
   \hline
   All-Reply & 0.48 & 0.49 & 0.64 & & & \\
        Linear-Reply & 0.09 & 0.11 & 0.21 & 0.18 & 0.14 & 0.13 \\
  \textbf{Our Model} & \textbf{0.75} & \textbf{0.77} & \textbf{0.76} & \textbf{0.78} & \textbf{0.68} & \textbf{0.60} \\
        Cross-domain &  \\
   \hline
  \end{tabular}
...
Our model was able to beat both baselines by a significant margin. We found 
these results  striking because the experiments of Section~\ref{sec:features} 
suggest that pairwise F1 and depth-based $B^3$ measurements are independent metrics.

Table~\ref{table:results} also reports our results with our \em{cross-domain}
experiments where we trained on our AskReddit data and tested on Hacker News. 

We utilized the popular Weka machine learning software to drive our
$(parent,child)$ binary classifier. Our results come after a long period of
validation over our development set. In particular, we optimize over the $\theta$ 
...
informal grid search over possible choices. Our best results across all metrics 
are reported using $\theta=0.1$, $n=25$ and a random forest (10 trees with 4
features each). This choice of $n$, creates a training set of over \textit{1.5
million} examples.In the future we would also like to try kernelized SVMs as an
option, but decided against more extensive cross-validation. 
